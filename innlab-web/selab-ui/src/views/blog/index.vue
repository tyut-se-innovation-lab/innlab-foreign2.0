        ...
        <div class="container">
                ...
                <section id="content"><!-- #content -->
                    ...
                    <div class="contentItem" v-for="item in actInfo.activityContent">
                        ...
                        <article v-if="item.subtitleType == 1">
                            <h2><a href="#">{{ item.subtitleName }}</a></h2>
                            <!-- <img v-for="img in item.resource" :src="img" alt="" class="alignleft" /> -->
                            <el-image v-for="(img, imgIndex) in item.resource" :key="img" :src="img" :zoom-rate="1.2"
                                :max-scale="7" :min-scale="0.2" :preview-src-list="item.resource"
                                :initial-index="imgIndex" fit="cover" />
                        </article>
                        <article v-if="item.subtitleType == 2" v-show="videoLoaded">
                            <h2><a href="#">{{ item.subtitleName }}</a></h2>
                            <video controls width="750" v-for="video in item.resource" :key="video">
            ...
            <div class="about">
                <aside id="sidebar"><!-- sidebar -->
                    <h3>日 常 活 动</h3>
                    <ul>
                        <li v-for="(item, index) in ActivitiesList">
                            <div class="titleNum">{{ index + 1 }}</div>
                            <span @click="linkTo(item.activityId)"> {{ item.activityTitle }}</span>
                        </li>
        ...
        for (const element of actInfo.value.activityContent) {
            if (element.resource) {
                const updatedResources = await Promise.all(
                    element.resource.map(async el => {
                        const link = await parseLanzouLink(el);
                        const res = await parseLanzouLink(link);
...
const ActivitiesList = ref<Array<{ activityId: number; activityTitle: string; activityIntroduction: string; headerImage: object }>>([]);
...
onMounted(() => {
    ...
    getActvityInfo();
    getActivitiesList().then(() => {
        handleVideoLoad(); // 在活动信息和列表加载后，触发视频加载
    });
